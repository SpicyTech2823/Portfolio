...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? "border-b border-slate-800 bg-slate-950/85 shadow-lg shadow-black/10 backdrop-blur-xl" : "bg-transparent"}`}
    >
      <nav className="mx-auto flex h-20 w-[min(1120px,calc(100%-2.5rem))] items-center justify-between">
        <a href="#home" className="font-bold tracking-[.18em] text-white">
          SS<span className="text-amber-300">.</span>
        </a>
        ...
        <div className="border-t border-slate-800 bg-slate-950 px-5 pb-5 md:hidden">
          {navItems.map((item, index) => (
            <a
              ...
              className="block border-b border-slate-800 py-3 text-sm text-slate-200"
              href={`#${ids[index]}`}
              key={item}
            >
              {item}
